                ...
                <div class="filter-controls">
                    ...
                    <div class="search-filter-inputs">
                        ...
                        <div class="filter-bar">
                            <select id="roleFilter" class="form-select"> <%-- Added form-select class --%>
...
// Submit Update Detail Form
                                            function submitUpdateDetailForm(event) {
                                                ...
                                                var formData = new FormData($("#updateAccountDetailForm")[0]);
                                                // Đảm bảo UserIdHidden được thêm vào formData nếu chưa có trong form HTML
                                                // formData.append("UserIdHidden", $("#DetailUserIdHidden").val()); // Thêm dòng này nếu cần

                                                console.log("Submitting Update Form..."); // Log bắt đầu Update
                                                ...
                                                $.ajax({
                                                    ...
                                                    dataType: "json",
                                                    success: function (response) {
                                                        console.log("UPDATE AJAX Success - Response Received:", response); // Log phản hồi Update
                                                        ...
                                                        if (response.success) {
                                                            ...
                                                            console.log("UPDATE Handling Error Response"); // Log vào nhánh lỗi Update
                                                            detailModal.show();
                                                            enableEditMode(); // Keep edit mode active
                                                            // Inside the success callback, within the 'else' block for response.success === false

                                                            if (response.field && response.message) {
                                                                console.log("UPDATE Error has field and message:", response.field, response.message);

                                                                let serverFieldName = response.field; // e.g., 'UserPhone'
                                                                let modalFieldId = serverFieldName; // Default to same name

                                                                // --- ADD THIS MAPPING LOGIC ---
                                                                if (serverFieldName === 'UserPhone') {
                                                                    modalFieldId = 'DetailUserPhone';
                                                                } else if (serverFieldName === 'UserEmail') {
                                                                    modalFieldId = 'DetailUserEmail';
                                                                } else if (serverFieldName === 'IdentityCard') {
                                                                    modalFieldId = 'DetailIdentityCard';
                                                                ...
                                                                console.log("UPDATE Calling displayError for mapped field:", modalFieldId); // Should now log 'DetailUserPhone' etc.

                                                                // Pass the CORRECT modalFieldId to displayError
                                                                displayError(modalFieldId, response.message);

                                                                // Update focus logic to use modalFieldId
//                                                                if (modalFieldId === 'DetailUserPhone') {
//                                                                    console.log("UPDATE Focusing on DetailUserPhone");
//                                                                    $('#DetailUserPhone').focus();
//                                                                } else 
                                                                    if (modalFieldId === 'DetailUserEmail') {
                                                                    $('#DetailUserEmail').focus();
                                                    ...
                                                    error: function (xhr, status, error) {
                                                        console.error("UPDATE AJAX Error:", status, error, xhr.responseText); // Log lỗi AJAX Update
                                                        displayError('DetailUserEmail', 'Error updating account: ' + error);
                                                    }
                                                });
                                            }
